        ...
        try {
            ...
            if (result.success) {
                ...
                setTimeout(async () => {
                    ...
                    // También puedes volver a renderizar la tabla aquí si no quieres recargar
                    window.location.reload();
                }, 1000);
            } else {
                mostrarNotificacion(result.message, 'danger');
...
function filtrarMedicamentos() {
    ...
    filas.forEach(fila => {
        ...
        // CORRECCIÓN: Obtener el estado de forma más robusta
        const estadoCell = fila.cells[6];
        let estado = '';
        
        // Buscar el badge dentro de la celda
        const badge = estadoCell.querySelector('.badge');
        if (badge) {
            estado = badge.textContent.trim();
        } else {
            // Si no hay badge, usar el texto directo de la celda
...
function mostrarNotificacion(mensaje, tipo) {
    // Remover notificaciones existentes primero
...
function generarReporteStock(logoBase64) {
    // Obtener los filtros actuales aplicados
    const searchTerm = document.getElementById('searchInput').value.toLowerCase();
    const estadoFilter = document.getElementById('filterEstado').value;
    const stockFilter = document.getElementById('filterStock').value;
    
    // Filtrar medicamentos igual que en la interfaz
    ...
    // Cuerpo de la tabla con los medicamentos filtrados
    ...
    if (medicamentosAImprimir.length === 0) {
        ...
        medicamentosAImprimir.forEach(med => {
            ...
            // Determinar clase de stock (igual que en la interfaz)
            let stockClass = '';
